...
class TriggerUpgradeSerializer(serializers.Serializer):
    ...
    schedule_time = serializers.CharField(
        ...
        allow_blank=True,
        help_text=(
            "Schedule upgrade for later. Accepted formats: "
            "'2026-02-16T05:05' (ISO), "
            "'2026-02-16 05:05 AM' (12h), "
            "'2026-02-16 17:05' (24h), "
            "'02/16/2026 05:05 PM' (US date), "
            "'16-02-2026 17:05' (DD-MM-YYYY), "
            "'Feb 16, 2026 05:05 PM' (month name). "
            "Timezone is based on server TIME_ZONE setting."
        ),
    )
    activate_after_distribute = serializers.BooleanField(
        default=True, help_text="Auto-activate after distribution completes"
    )
    cleanup_flash = serializers.BooleanField(
        default=False, help_text="Clean up flash storage after upgrade"
    )
    ...
    remarks = serializers.CharField(
        required=False,
        allow_blank=True,
        help_text="Optional global remarks or RFC number",
    )
    device_remarks = serializers.DictField(
        child=serializers.CharField(allow_blank=True),
        required=False,
        help_text="Map of device ID to specific remarks. Overrides global remarks.",
    )
...
class CancelUpgradeSerializer(serializers.Serializer):
    ...
    job_ids = serializers.ListField(
        child=serializers.IntegerField(), help_text="List of job IDs to cancel"
    )
...
@extend_schema(
    ...
    examples=[
        OpenApiExample(
            "Parallel Upgrade (Immediate)",
            value={
                "devices": [1, 2, 3],
                "execution_mode": "parallel",
                "auto_select_image": True,
                "remarks": "RFC-12345: Standard Upgrade",
                "device_remarks": {
                    "1": "RFC-12345: Standard Upgrade (Core)",
                    "2": "RFC-12345: Standard Upgrade (Access)"
                }
            },
            request_only=True,
        ),
        OpenApiExample(
            ...
            value={
                ...
                "execution_mode": "parallel",
                "auto_select_image": True,
                "schedule_time": "2026-02-16 10:30 PM",
                "remarks": "RFC-67890: Maintenance Window"
            },
            request_only=True,
        )
    ],
...
def trigger_upgrade_pipeline(request):
    ...
    global_remarks = request.data.get("remarks", "")
    device_remarks = request.data.get("device_remarks", {})
    ...
    # Validate device models are supported
    ...
    for device in devices:
        ...
        if model_name and model_name not in settings.SUPPORTED_DEVICE_MODELS:
            unsupported_devices.append(f"{device.hostname} ({model_name})")

    if unsupported_devices:
        return Response(
            {
                "error": "Device model not supported",
                "message": f"The following devices have models not in supported list: {', '.join(unsupported_devices)}",
            },
            status=400,
        )
    ...
    if schedule_time:
        try:
            # Try multiple date/time formats for flexibility
            parsed_dt = None
            formats = [
                "%Y-%m-%dT%H:%M:%S",    # ISO: 2026-02-16T05:05:00
                "%Y-%m-%dT%H:%M",        # ISO short: 2026-02-16T05:05
                "%Y-%m-%d %H:%M:%S",     # 2026-02-16 05:05:00
                "%Y-%m-%d %H:%M",        # 2026-02-16 05:05
                "%Y-%m-%d %I:%M %p",     # 2026-02-16 05:05 AM
                "%Y-%m-%d %I:%M:%S %p",  # 2026-02-16 05:05:00 AM
                "%m/%d/%Y %I:%M %p",     # 02/16/2026 05:05 AM
                "%m/%d/%Y %H:%M",        # 02/16/2026 17:05
                "%d-%m-%Y %H:%M",        # 16-02-2026 17:05
                "%d-%m-%Y %I:%M %p",     # 16-02-2026 05:05 AM
                "%b %d %Y %I:%M %p",     # Feb 16 2026 05:05 AM
                "%b %d, %Y %I:%M %p",    # Feb 16, 2026 05:05 AM
            ]

            # First try fromisoformat (handles timezone-aware strings)
            try:
                parsed_dt = datetime.fromisoformat(schedule_time.replace("Z", "+00:00"))
            ...
            if not parsed_dt:
                for fmt in formats:
                    try:
                        parsed_dt = datetime.strptime(schedule_time, fmt)
                        break
                    except ValueError:
                        continue
                ...
                return Response(
                    {
                        ...
                        "message": (
                            "Could not parse schedule_time. Supported formats: "
                            "'2026-02-16T05:05', '2026-02-16 05:05 AM', "
                            "'02/16/2026 5:05 PM', 'Feb 16, 2026 5:05 PM'"
                        ),
                    },
                    status=400,
                )
            ...
            if parsed_dt.tzinfo is None:
                distribution_time = timezone.make_aware(parsed_dt)
            else:
                distribution_time = parsed_dt

        except (ValueError, TypeError) as e:
            return Response(
                {
                    "error": "Invalid schedule_time",
                    "message": f"Could not parse schedule_time: {str(e)}",
                },
                status=400,
            )
    ...
    for device in devices:
        ...
        # Determine specific remarks for this device
        # Check both str(id) and int(id) just in case
        specific_remark = device_remarks.get(str(device.id)) or device_remarks.get(device.id) or global_remarks
    ...
    if not schedule_time and created_jobs:
        if execution_mode == "sequential":
            ...
            for job in created_jobs:
                ...
                t.start()
            logger.info(
                f"Started parallel upgrade pipeline with {len(created_jobs)} jobs"
            )
...
def get_upgrade_status(request):
    ...
    for job_data in serializer.data:
        ...
        if steps:
            current_step = next(
                (s for s in steps if s.get("status") in ["running", "pending"]), None
            )
            job_data["current_step"] = (
                current_step["name"] if current_step else "Completed"
            )
        else:
    ...
    return Response({"count": len(jobs_data), "jobs": jobs_data})
...
@extend_schema(
    ...
    examples=[
        OpenApiExample("Cancel Jobs", value={"job_ids": [1, 2, 3]}, request_only=True)
    ],
)
@api_view(["POST"])
@permission_classes([IsAuthenticated])
def cancel_upgrade(request):
    ...
    if not job_ids:
        return Response(
            {"error": "Missing job_ids", "message": "Please provide job IDs to cancel"},
            status=400,
        )
    ...
    return Response(
        {"status": "success", "cancelled": cancelled_count, "job_ids": job_ids}
    )
